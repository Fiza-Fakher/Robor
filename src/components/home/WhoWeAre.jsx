    ...
    <>
      <div className="pb-25 overflow-hidden">
        <section className="pt-20 pb-12">
          ...
          <h2 className="flex flex-col text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold pt-6 sm:pt-8 ml-3 sm:ml-6 leading-snug">
          ...
          {/* Cards Section */}
          <div className="flex flex-col sm:flex-row justify-center items-start gap-4 px-4 sm:px-0">
            {whoCards.map((items, index) => (
              <div
                key={index}
                className="flex flex-col items-center sm:items-start lg:ml-4 lg:mr-4"
              >
                <img
                  src={items.image}
                  alt=""
                ...
                <h2 className="text-xl sm:text-2xl pt-4 font-bold text-center sm:text-left">
                  {items.title}
                </h2>
                <p className="pt-3 sm:pt-4 text-base sm:text-lg text-gray-600 w-full sm:w-130 text-center sm:text-left">
                  {items.des}
                </p>
              </div>
            ))}
        ...
        {/* Marquee Section */}
        <section className="bg-[#F4F1FC] text-black flex justify-start items-center w-full h-20 sm:h-24 lg:h-30 mt-10 sm:mt-12 text-lg sm:text-2xl capitalize font-bold">
          <FastMarquee>
            {whoContent.map((items, index) => (
              <div key={index} className="flex justify-start items-center ml-6">
                <h1>{items}</h1>
